...
$mainframe->registerEvent( 'onSearch', 'plgSearchContentMatch' );
$mainframe->registerEvent( 'onSearchAreas', 'plgSearchContentMatchAreas' );
 ...
 */
function &plgSearchContentMatchAreas()
{
	static $areas = array(
		'content' => 'Pages'
	);
 ...
 */
function plgSearchContentMatch( $text, $phrase='', $ordering='', $areas=null )
{
	global $mainframe;
	...
	switch ($phrase) {
		case 'exact':
			...
			$text		= $db->Quote( $db->getEscaped( $text, true ), false );
			$wheres2 	= array();
			$wheres2[] 	= 'MATCH(a.title) AGAINST('.$text.' IN BOOLEAN MODE)';
			$wheres2[] 	= 'MATCH(a.introtext) AGAINST('.$text.' IN BOOLEAN MODE)';
			$wheres2[] 	= 'MATCH(a.fulltext) AGAINST('.$text.' IN BOOLEAN MODE)';
			$wheres2[] 	= 'MATCH(a.metakey) AGAINST('.$text.' IN BOOLEAN MODE)';
			$wheres2[] 	= 'MATCH(a.metadesc)AGAINST('.$text.' IN BOOLEAN MODE)';
		...
		default:
			...
			foreach ($words as $word) {
				...
				$word		= $db->Quote( $db->getEscaped( $word, true ), false );
				$wheres2 	= array();
				$wheres2[] 	= 'MATCH(a.title) AGAINST('.$word.' IN BOOLEAN MODE)';
				$wheres2[] 	= 'MATCH(a.introtext) AGAINST('.$word.' IN BOOLEAN MODE)';
				$wheres2[] 	= 'MATCH(a.fulltext) AGAINST('.$word.' IN BOOLEAN MODE)';
				$wheres2[] 	= 'MATCH(a.metakey) AGAINST('.$word.' IN BOOLEAN MODE)';
				$wheres2[] 	= 'MATCH(a.metadesc) AGAINST('.$word.' IN BOOLEAN MODE)';
